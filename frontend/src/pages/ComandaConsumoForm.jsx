...
const ComandaConsumoForm = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const mensagem =
          error.apiMessage || "Erro ao carregar dados da comanda";
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const itemData = {
        ...
        valor_unitario:
          produtos.find((p) => p.id === parseInt(data.produto_id))
            ?.valor_unitario || 0,
      };
      if (editingItemId) {
  ...
  const handleRemoveItem = (item) => {
    ...
    showConfirm(
      "Remover Item",
      `Tem certeza que deseja remover "${pNome}", quantidade ${item.quantidade}?`,
      async () => {
        try {
