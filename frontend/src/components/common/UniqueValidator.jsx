...
const UniqueValidator = ({
  ...
  return (
    <Dialog
      ...
      aria-labelledby="unique-dialog-title"
      aria-describedby="unique-dialog-description"
      maxWidth="sm"
      fullWidth
    >
      <DialogTitle
        id="unique-dialog-title"
        sx={{
          fontWeight: 600,
          color: "white",
          ...
          <Box sx={{ p: 2, backgroundColor: "grey.100", borderRadius: 1 }}>
            ...
            {existingRecord.cpf && (
              <Typography variant="body2" color="text.secondary">
                CPF: {existingRecord.cpf}
              </Typography>
            )}
            {existingRecord.matricula && (
              <Typography variant="body2" color="text.secondary">
                Matrícula: {existingRecord.matricula}
              </Typography>
            )}
            {existingRecord.telefone && (
              <Typography variant="body2" color="text.secondary">
                Telefone: {existingRecord.telefone}
              </Typography>
            )}
            ...
            {existingRecord.descricao && (
              <Typography variant="body2" color="text.secondary">
                Descrição: {existingRecord.descricao}
              </Typography>
            )}
            {existingRecord.valor_unitario && (
              <Typography variant="body2" color="text.secondary">
                Valor Unitário: R${" "}
                {Number(existingRecord.valor_unitario).toFixed(2)}
              </Typography>
            )}
          </Box>
        )}
...
export const useFieldValidation = (
  service,
  currentId = null,
  validationMethod = "checkCpfExists",
) => {
  ...
  const validateField = async (fieldValue) => {
    ...
    try {
      const existingRecord = await service[validationMethod](
        fieldValue,
        currentId,
      );
      if (existingRecord) {
