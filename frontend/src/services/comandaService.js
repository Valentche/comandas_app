...
const comandaService = {
  ...
  list: async (params = {}) => {
    ...
    if (comanda !== undefined && comanda !== null && comanda !== "")
      queryParams.append("comanda", comanda);
    ...
    if (
      ...
      queryParams.append("funcionario_id", funcionario_id);
    if (cliente_id !== undefined && cliente_id !== null && cliente_id !== "")
      queryParams.append("cliente_id", cliente_id);
    if (data_inicio !== undefined && data_inicio !== null && data_inicio !== "")
      queryParams.append("data_inicio", data_inicio);
    if (data_fim !== undefined && data_fim !== null && data_fim !== "")
  ...
  checkEmUso: async (comanda, id = null) => {
    ...
    const response = await api.get(
      `${COMANDA.LIST}?${new URLSearchParams(params).toString()}`,
    );
  ...
  fechar: async (id) => {
    const response = await api.put(COMANDA.UPDATE.replace(":id", id), {
      status: 1,
    });
    return response.data;
  },
  ...
  addItem: async (comandaId, itemData) => {
    const response = await api.post(
      COMANDA.ADD_ITEM.replace(":id", comandaId),
      itemData,
    );
    return response.data;
  },
  ...
  updateItem: async (itemId, itemData) => {
    const response = await api.put(
      COMANDA.UPDATE_ITEM.replace(":id", itemId),
      itemData,
    );
    return response.data;
  },
  ...
  removeItem: async (itemId) => {
    const response = await api.delete(
      COMANDA.REMOVE_ITEM.replace(":id", itemId),
    );
    return response.data;
  },
};
